...
try : 
    ...
    sys.stderr.write("booleanparamreadout: unable to import dc_value class... widget will not operate correctly")
    dc_value=None
    pass
...
class booleanreadout(gtk.CheckButton,paramhandler):
    ...
    def __init__(self):
        ...
        paramhandler.__init__(self,super(booleanparamreadout,self),self.__proplist)
    ...
    def set_fixed(self,fixed,fixedvalue=None,fixeddisplayfmt=None):
        ...
        if fixed:
            self.fixedvalue=bool(fixedvalue)
            self.set_state(self.STATE_FIXED)
            self.changedinhibit=True
            self.set_active(self.fixedvalue)
            self.changedinhibit=False
            pass
    ...
    def newvalue(self,param,condition):
        # (condition not used)
        if self.state==self.STATE_WAITING or self.state==self.STATE_FIXED:
            return
        self.changedinhibit=True
        self.set_active(param.dcvalue.value())
        self.changedinhibit=False
    ...
    def dc_gui_init(self,guistate):
        ...
        if self.paramdb is None:  # allow manual initialization of paramdb, in case we are to use a non-default paramdb
            ...
            pass
        self.__booleanparamreadout_unique=[]
    ...
    def sync_to_paramdb(self):
        ...
        self.changedinhibit=True
        self.set_active(self.param.dcvalue.value())
        self.changedinhibit=False
    ...
    def requestvalcallback(self,param,requestid,errorstr,result):
        # print "booleanparam requestvalcallback. self.state=%d" % (self.state)
...
gobject.type_register(booleanparamreadout)  # required since we are defining new properties/signals
