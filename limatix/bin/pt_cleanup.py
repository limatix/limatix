...
def cleanup_obsolete_tags(xlpdocu):
    ...
    assert(all([targethref.fragless()==xlphref for (target,steptag,targethref) in validtarget_step_and_targethref])) # IF THIS ASSERTION TRIPS, check if xlp/pro file has lip:target elements with bad xlink:href self-references that start with .# . If so, run /usr/local/bin/xlp_target_fixup on the .xlp or .pro file(s). 
