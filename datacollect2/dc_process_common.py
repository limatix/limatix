...
def open_or_lock_output(prxdoc,out,overall_starttime,copyfileinfo=None):
    ...
    try: 
        ...
        process_el=out.output.addelement(outputroot,"dcp:process")
        wcb_el=out.output.addelement(process_el,"dcp:wascontrolledby")
        prxfile_el=provenance.reference_file(out.output,wcb_el,"dcp:prxfile",outputroot,prxdoc.get_filehref().value(),"info")
        out.output.setattr(prxfile_el,"dcp:timestamp",datetime.datetime.fromtimestamp(os.path.getmtime(prxdoc.get_filehref().getpath()),dg_timestamp.UTC()).isoformat())
    
        provenance.write_timestamp(out.output,process_el,"dcp:starttimestamp",overall_starttime)
        ...
        if copyfileinfo is not None:
            ...
            cfprocess_el=out.output.addelement(process_el,"dcp:process")
            # have dcp:used point to the input file we copied from 
            provenance.reference_file(out.output,cfprocess_el,"dcp:used",outputroot,out.inputfilehref.value(),warnlevel="warning",fragcanonsha256=canonhash)
            provenance.write_action(out.output,cfprocess_el,"_copy_input_file")
            provenance.write_timestamp(out.output,cfprocess_el,"dcp:starttimestamp",cf_starttime)
            provenance.write_timestamp(out.output,cfprocess_el,"dcp:finishtimestamp")
...
def finalize_output_file(prxdoc,outputdict,inputfilehref):
    ...
    try: 
        ...
        provenance.write_timestamp(out.output,process_el,"dcp:finishtimestamp")
        pass
    finally:
        out.output.unlock_rw()  # free output lock
        pass
...
class outputdoc(object):
    ...
    processpath=None # path to the dcp:process tag for our overall process
